@echo off
set "Asset_TA_Path=E:\WorkSpace\AwesomeLive\Asset_TA"
...
mklink /j "%CURRENT_DIR%\..\Content\Prop" "%Asset_TA_Path%\Prop"
if %errorLevel% equ 0 (
    echo 成功创建目录硬链接！
) else (
    echo 创建目录硬链接失败，错误代码：%errorLevel%
...
mklink /j "%CURRENT_DIR%\..\Content\ScriptAndBlueprint" "%Asset_TA_Path%\ScriptAndBlueprint"
if %errorLevel% equ 0 (
    echo 成功创建目录硬链接！
) else (
    echo 创建目录硬链接失败，错误代码：%errorLevel%
